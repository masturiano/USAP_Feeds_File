...
if($ssh = ssh2_connect('174.129.233.96', 22)) {
    if(ssh2_auth_password($ssh, 'masturiano', 'masturianousap1q2w')) 
    {
		...
		if ($dirhandler = opendir("ssh2.sftp://$sftp/data/aopi/feeds_longbow/output/dam_ci_details/latest/")) 
		{
			...
			{
				// Remote stream
		        if (!$remoteStream = @fopen("ssh2.sftp://$sftp/data/aopi/feeds_longbow/output/dam_ci_details/latest/$file_name", 'r')) {
		            throw new Exception("Unable to open remote file: $file_name");
		        } 
		        echo $fileSize = filesize("ssh2.sftp://$sftp/data/aopi/feeds_longbow/output/dam_ci_details/latest/$file_name");
